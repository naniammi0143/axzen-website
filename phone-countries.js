...
export function getPhoneCountry(iso = DEFAULT_PHONE_COUNTRY) {
  return countryByIso.get(iso) || countryByIso.get(DEFAULT_PHONE_COUNTRY);
}

export function nationalNumber(value) {
  return String(value || "").replace(/\D/g, "").replace(/^0+/, "");
}

export function buildE164(iso, rawInput) {
  ...
  let digits = nationalNumber(rawInput);
...
export function isValidE164(e164, iso = DEFAULT_PHONE_COUNTRY) {
  if (!/^\+\d{8,15}$/.test(e164)) return false;
  if (iso === "IN") return /^\+91\d{10}$/.test(e164);
  return true;
}

export function otpAuthErrorMessage(error) {
...
export function fillCountrySelects(root = document) {
  root.querySelectorAll("[data-country-select]").forEach((select) => {
    const current = select.value || DEFAULT_PHONE_COUNTRY;
    select.innerHTML = PHONE_COUNTRIES.map(
      (country) =>
...
export function phoneFromRoot(root) {
  const iso = root.querySelector("[data-country-select]")?.value || DEFAULT_PHONE_COUNTRY;
  const input =
    root.querySelector("[name='phone']") ||
    root.querySelector("[name='mobile']") ||
    root.querySelector("[data-owner-phone]");
  return {
    iso,
    e164: buildE164(iso, input?.value || ""),
  };
}
